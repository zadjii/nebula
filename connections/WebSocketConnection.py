...
class MyBigFuckingLieServerProtocol(WebSocketServerProtocol):
    ...
    def __init__(self):
        ...
        self._internal_client_socket = None # socket.socket()
    ...
    def onConnect(self, request):
        ...
        self._internal_client_socket.connect(('localhost', HOST_WS_PORT+1)) # fixme constant
        addr = request.peer
        MyBigFuckingLieServerProtocol.net_thread.add_ws_conn(self)
        mylog('what the hell is going on')
        # self._internal_client_socket.connect(('localhost', HOST_PORT))  # fixme constant
        # self.ws_conn = WebsocketConnection(self._internal_client_socket, self)
        # MyBigFuckingLieServerProtocol.net_thread.
